    ...
    this.showTaskRunSetCreate = function (ev, ips, create_callback) {
        $mdDialog.show({
                controller: function DialogController($scope, $timeout, $http, $mdDialog, $mdToast) {
                    ...
                    $scope.boz = function () {
                        console.log($scope.selected_ips);
                    };
                    ...
                    $scope.$watch('show_connected', function () {
                        $scope.table_nodes = [];
                        angular.forEach($scope.all_nodes, function (item) {
                            if ($scope.show_connected == 'All')
                                $scope.table_nodes.push(item);
                            if ($scope.show_connected == 'true' && item.connected)
                                $scope.table_nodes.push(item);
                            if ($scope.show_connected == 'false' && !item.connected)
                                $scope.table_nodes.push(item);
